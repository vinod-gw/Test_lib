...
function dhtmlXCalendarObject(f, k) {
	...
	this.conf = {
		skin : (k || window.dhx4.skin
				...
				|| window.dhx4.skinDetect("dhtmlxcalendar") || "dhx_skyblue"),
	...
	this.contDates[this.conf.touch ? "ontouchend" : "onclick"] = function(p) {
		...
		if (l._date != null && !l._css_dis) {
			...
			if (e._activeInp && e.i[e._activeInp]
					&& e.i[e._activeInp].input != null) {
				e.i[e._activeInp].input.value = e._dateToStr(new Date(
						e._activeDate.getTime()))
			}
			if (!e._hasParent) {
				e._hide()
			}
	...
	if (!this.conf.touch) {
		this.contDates.onmouseover = function(l) {
			...
			if (a._date != null) {
				...
				e._lastHover = a;
				e._evOnMouseOver([
						new Date(a._date.getFullYear(), a._date.getMonth(),
								a._date.getDate(), 0, 0, 0, 0), l ]);
				a = null
			}
		};
	...
	this._clearDayHover = function(a) {
		...
		e._evOnMouseOut([
				new Date(this._lastHover._date.getFullYear(),
						this._lastHover._date.getMonth(), this._lastHover._date
								.getDate(), 0, 0, 0, 0), a ]);
		this._lastHover = null
	};
	for ( var b = 0; b < 6; b++) {
	...
	this._drawMonth = function(s) {
		...
		if (isNaN(s.getFullYear())) {
			s = new Date(this._activeMonth.getFullYear(), this._activeMonth
					.getMonth(), 1, 0, 0, 0, 0)
		}
		this._activeMonth = new Date(s.getFullYear(), s.getMonth(), 1, 0, 0, 0,
				0);
		this._activeDateCell = null;
		...
		var z = s.getMonth();
		var A = new Date(this._activeDate.getFullYear(), this._activeDate
		...
		for ( var l = 0; l < 6; l++) {
			...
			for ( var y = 0; y <= 7; y++) {
				if (y == 0) {
					var x = this.getWeekNumber(new Date(p.getFullYear(), p
							.getMonth(), p.getDate() + o, 0, 0, 0, 0));
					this.contDates.childNodes[l].childNodes[y].innerHTML = "<div class='dhtmlxcalendar_label'>"
							+ x + "</div>"
				} else {
					var a = new Date(p.getFullYear(), p.getMonth(), p.getDate()
							+ o, 0, 0, 0, 0);
					var u = a.getDay();
					var n = a.getTime();
					...
					if (this._tipData[n] != null) {
						if (this._tipData[n].usePopup
							...
							this.contDates.childNodes[l].childNodes[y]
					...
					this.contDates.childNodes[l].childNodes[y].innerHTML = "<div class='"
							+ t + "'>" + a.getDate() + "</div>";
					this.contDates.childNodes[l].childNodes[y]._date = new Date(
							n);
					this.contDates.childNodes[l].childNodes[y]._q = l;
					this.contDates.childNodes[l].childNodes[y]._w = y;
					this.contDates.childNodes[l].childNodes[y]._css_month = (a
							.getMonth() == z);
					...
					this.contDates.childNodes[l].childNodes[y]._css_dis = this
							._isOutOfRange(n);
	...
	this._initSelector = function(s, n) {
		...
		if (!this._sel) {
			...
			this._sel.firstChild.firstChild.firstChild.childNodes[0][this.conf.touch ? "ontouchstart"
					: "onclick"] = function(q) {
				q = q || event;
				q.cancelBubble = true;
				e._scrollYears(-1)
			};
			this._sel.firstChild.firstChild.firstChild.childNodes[2][this.conf.touch ? "ontouchstart"
					: "onclick"] = function(q) {
				q = q || event;
				q.cancelBubble = true;
				e._scrollYears(1)
		...
		if (s == "month") {
			...
			this.msCont[this.conf.touch ? "ontouchstart" : "onclick"] = function(
					v) {
				v = v || event;
				v.cancelBubble = true;
				var q = (v.target || v.srcElement);
		...
		if (s == "year") {
			...
			this.ysCont[this.conf.touch ? "ontouchstart" : "onclick"] = function(
					v) {
				v = v || event;
				v.cancelBubble = true;
				var q = (v.target || v.srcElement);
		...
		if (s == "hours") {
			...
			this.hsCont[this.conf.touch ? "ontouchstart" : "onclick"] = function(
					v) {
				v = v || event;
				v.cancelBubble = true;
				var q = (v.target || v.srcElement);
		...
		if (s == "minutes") {
			...
			this.rsCont[this.conf.touch ? "ontouchstart" : "onclick"] = function(
					w) {
				w = w || event;
				w.cancelBubble = true;
		...
		if (this.checkEvent("onBeforeChange")) {
			if (!this.callEvent("onBeforeChange", [ new Date(a.getFullYear(), a
				...
				if (this.i != null && this._activeInp != null
					...
					this.i[this._activeInp].input.value = this
							.getFormatedDate()
				}
				return
			}
	...
	this.getWeekNumber = function(o) {
		...
		var v = 604800000;
		var a = new Date(o.getFullYear(), o.getMonth(), o.getDate() + 1, 0, 0,
				0, 0);
	...
	this._show = function(q, a) {
		...
		if (!q) {
			...
			var o = Math.max((l ? document.documentElement : document
					.getElementsByTagName("html")[0]).scrollTop,
					document.body.scrollTop);
			var m = o
					+ (l ? Math.max(document.documentElement.clientHeight || 0,
							document.documentElement.offsetHeight || 0,
							document.body.clientHeight || 0)
							: window.innerHeight);
			if (this.pos == "right") {
				this.base.style.left = this._getLeft(n) + n.offsetWidth + "px";
				this.base.style.top = Math.min(this._getTop(n), m
	...
	this._posGetOffset = function(a) {
		return this[a.getBoundingClientRect ? "_posGetOffsetRect"
				: "_posGetOffsetSum"](a)
	};
	...
	if (typeof (window.addEventListener) == "function") {
		document.body.addEventListener((this.conf.touch ? "touchstart"
				: "click"), e._doOnClick, false);
		...
		if (this.i[l].button != null) {
			...
			if (typeof (window.addEventListener) == "function") {
				this.i[l].button.addEventListener(
						(this.conf.touch ? "touchstart" : "click"),
	...
	this.unload = function() {
		...
		if (typeof (window.addEventListener) == "function") {
			document.body.removeEventListener((this.conf.touch ? "touchstart"
					: "click"), e._doOnClick, false);
